...
function updateUI(data) {
    ...
    ipContainer.innerHTML = ''; // 清空之前内容

    if (data.ip_address && data.ip_address !== '无公网IP') {
        const ipv4Html = `
            <div class="d-flex align-items-center mt-1">
                <span class="badge bg-light text-secondary me-2" style="font-size: 0.7rem; border: 1px solid #dee2e6;">IPv4</span>
                <div class="info-value text-break" style="font-family: monospace; font-size: 0.9rem;">${data.ip_address}</div>
            </div>`;
        ipContainer.innerHTML += ipv4Html;
    }

    if (data.ipv6_address) {
        const ipv6Html = `
            <div class="d-flex align-items-center mt-1">
                <span class="badge bg-light text-secondary me-2" style="font-size: 0.7rem; border: 1px solid #dee2e6;">IPv6</span>
                <div class="info-value text-break" style="font-family: monospace; font-size: 0.9rem;">${data.ipv6_address}</div>
            </div>`;
        ipContainer.innerHTML += ipv6Html;
    }

    if (ipContainer.innerHTML === '') {
        ipContainer.innerHTML = '<div class="text-muted small">无公网IP</div>';
    }

    document.getElementById('used-val').innerText = data.used;

    // 历史流量显示
    const prevTrafficEl = document.getElementById('prev-traffic');
    if (data.prev_traffic && data.prev_traffic.length > 0) {
        prevTrafficEl.style.display = 'block';
        prevTrafficEl.innerHTML = data.prev_traffic.map(item => `<div>${item.month}: ${item.used} GB</div>`).join('');
    } else {
        prevTrafficEl.style.display = 'none';
    }
    ...
    if (data.bill_amount !== undefined) {
        ...
        // 历史账单显示
        const prevBillsEl = document.getElementById('prev-bills');
        if (data.prev_bills && data.prev_bills.length > 0) {
            prevBillsEl.style.display = 'block';
            let historyHtml = '<div class="fw-bold mb-1" style="font-size: 0.75rem;">历史账单：</div>';
            historyHtml += data.prev_bills.map(item => `<div style="font-size: 0.75rem;">${item.month}: ${currencySymbol}${parseFloat(item.amount).toFixed(2)}</div>`).join('');
            prevBillsEl.innerHTML = historyHtml;
        } else {
            prevBillsEl.style.display = 'none';
        }
    }
