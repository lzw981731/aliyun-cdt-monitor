...
* **📊 仪表盘可视化**：
    ...
    * **隐私保护**：前端显示的 IPv4/IPv6 地址自动脱敏（如 `1.2.xxx.3` 或 `240b:...:xxxx`）。
    * **双栈增强支持**：支持并智能对齐显示 IPv4 和 IPv6 地址 (通过网络接口API穿透查询，解决系统未返回问题)。
* **🛡️ 双重熔断保护**：
...
* **🔔 消息通知与免打扰**：
    ...
    * 支持流量预警、关机通知、费用熔断警告。
    * **重启防打扰休眠机制**：开机重试连续失败（默认3次）后将进入智能推送休眠模式，暂停告警以防打扰，直到系统真正启动成功后自动重置。
* **⏰ 定时与保活**：
    * 支持配合 Crontab 设置定时开机/关机计划。
    * **关机强起保活**：支持检测机器状态，如果非熔断机制关机，会自动强制重新开机以保活。
...
为了实现自动监控和熔断保护，你需要配置服务器的定时任务，每10分钟执行一次检测。

**Linux Crontab 示例：**

```bash

# 每分钟请求一次 API，触发监控逻辑（建议设置10分钟）
